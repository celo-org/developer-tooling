...
import { Account, Chain, SimulateContractParameters, WalletClient } from 'viem'
...
export function accountsWrapper() {
  return (client: WalletClient, chain: Chain, account: Account) => {
    const basicParams = {
      abi: accountsABI,
      address: '0x', // todo either get with the registry or from static contracts on chain
      chain,
      account,
    } as const

    return {
      createAccount: async function createAccount() {
        return simwrite({ args: [], functionName: 'createAccount' })
      },
      setMetadataURL: async function setMetadataURL(url: string) {
        return simwrite({ args: [url], functionName: 'setMetadataURL' })
      },
      setPaymentDelegation: async function setPaymentDelegation(
        address: StrongAddress,
        amount: bigint
      ) {
        return simwrite({ args: [address, amount], functionName: 'setPaymentDelegation' })
      },
    }

    async function simwrite({
      args,
      functionName,
    }: Pick<SimulateContractParameters<typeof accountsABI>, 'functionName' | 'args'>) {
