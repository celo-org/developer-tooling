...
testWithAnvilL2('account:new cmd', (client) => {
  ...
  it('generates mnemonic and lets people know which derivation path is being used when called with no flags', async () => {
    await testLocallyWithWeb3Node(NewAccount, [], client)
  ...
  it("when called with --derivationPath eth it generates mnemonic using m/44'/60'/0'", async () => {
    await testLocallyWithWeb3Node(NewAccount, ['--derivationPath', 'eth'], client)
  ...
  it(`when called with --derivationPath celoLegacy it generates with "m/44'/52752'/0'"`, async () => {
    await testLocallyWithWeb3Node(NewAccount, ['--derivationPath', 'celoLegacy'], client)
  ...
  describe('bad data --derivationPath', () => {
    it(`with invalid alias "notARealPath"  throws"`, async () => {
      await expect(
        testLocallyWithWeb3Node(NewAccount, ['--derivationPath', 'notARealPath'], client)
    ...
    it(`with invalid bip44 throws"`, async () => {
      await expect(
        testLocallyWithWeb3Node(NewAccount, ['--derivationPath', 'm/44/1/1/2/10'], client)
      ).rejects.toThrowErrorMatchingInlineSnapshot(`
        "Parsing --derivationPath 
    ...
    it('with bip44 with changeIndex 4 throws', async () => {
      await expect(
        testLocallyWithWeb3Node(NewAccount, ['--derivationPath', "m/44'/52752'/0/0'"], client)
      ).rejects.toThrowErrorMatchingInlineSnapshot(`
        "Parsing --derivationPath 
    ...
    it('with bip44 including changeIndex 4 and addressIndex 5 throws', async () => {
      await expect(
        testLocallyWithWeb3Node(NewAccount, ['--derivationPath', "m/44'/52752'/0/0/0'"], client)
      ).rejects.toThrowErrorMatchingInlineSnapshot(`
        "Parsing --derivationPath 
    ...
    it(`with path ending in "/" removes the slash`, async () => {
      await testLocallyWithWeb3Node(NewAccount, ['--derivationPath', "m/44'/60'/0'/"], client)
  ...
  describe('when called with --mnemonicPath', () => {
    ...
    it('generates using eth derivation path', async () => {
      await testLocallyWithWeb3Node(NewAccount, [`--mnemonicPath`, MNEMONIC_PATH], client)
